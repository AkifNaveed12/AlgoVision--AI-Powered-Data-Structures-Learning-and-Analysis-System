...
def get_headers():
    ...
    # If self-hosting and using an auth token, you could use X-Auth-Token
...
async def execute_code(source_code: str, language_id: int, stdin: str = "") -> dict:
    ...
    """
    # RapidAPI requires a key, self-hosted might not.
    if "rapidapi.com" in settings.JUDGE0_API_URL and not settings.JUDGE0_API_KEY:
        raise HTTPException(
            status_code=503,
            detail="Code execution service not configured yet (JUDGE0_API_KEY missing for RapidAPI)"
        )
