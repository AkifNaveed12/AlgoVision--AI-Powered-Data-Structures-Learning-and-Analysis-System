...
Judge0 (RapidAPI)   https://rapidapi.com/judge0-official/api/judge0-ce → Subscribe (free tier)
                    Copy: X-RapidAPI-Key and X-RapidAPI-Host from the endpoint page
```
...
> Owner: Akif (Judge0 service) + Arslan (editor UI)
> Goal: Users write code in browser, execute via Judge0, see real output.

---

### T6.0 — Judge0 Service

**File: `backend/services/judge0_service.py`**
...
**File: `backend/routers/compiler.py`** — `POST /execute` (protected), calls `judge0_service.execute_code`
...
- Language info strip showing current language and Judge0 ID
...
- [ ] Judge0 timeout (deliberately long loop) → timeout message shown
...
```bash
git commit -m "feat(compiler): Judge0 code execution integration

- judge0_service.py: async submit + poll with base64 encoding/decoding
...
- Supports Python, C++, Java, JavaScript, C (Judge0 language IDs)"
...
> Goal: 10 seeded problems, code submission, Judge0 validation, AI hints.
...
- `POST /practice/submit` (protected):
  ...
  2. Execute user's code via `judge0_service.execute_code`
  3. Compare `stdout.strip()` with `expected_output.strip()`
  4. Determine status: `Accepted` or `Wrong Answer` (or `Runtime Error`/`Compile Error` from Judge0)
...
```bash
git commit -m "feat(practice): practice problem system with Judge0 validation
...
- Submit: Judge0 execution + expected_output comparison + attempt save
...
5. Judge0 connection verified: `POST /execute` runs Python code
...
#   2. Sign up / Log in
#   3. Search for "Judge0 CE" in API marketplace
#   4. Subscribe to the FREE "Basic" plan (100 submissions/day)
#   5. Go to the Judge0 CE API page → "Endpoints" tab
#   6. Copy "X-RapidAPI-Key" from the request headers shown → JUDGE0_API_KEY
#   7. The host is always: judge0-ce.p.rapidapi.com
JUDGE0_API_URL=https://judge0-ce.p.rapidapi.com
...
fix(judge0): handle base64 decode failure on empty stdout
