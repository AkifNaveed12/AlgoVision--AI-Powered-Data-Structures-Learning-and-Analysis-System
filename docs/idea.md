...
| **Code Execution** | Judge0 API | Sandboxed multi-language compiler |
...
│  │ performance_tracker.py  │    │  │ judge0_service.py      │  │
│  └─────────────────────────┘    │  │ report_service.py      │  │
│                                  │  └────────────────────────┘  │
└─────────────────────┬────────────┴──────────────┬───────────────┘
                      │                            │
          ...
          │                      │    │                       │
          │  - users             │    │  Groq API (AI)        │
          │  - practice_problems │    │  Judge0 API (compiler) │
          │  - algorithm_runs    │    │                       │
          │  - reports           │    └───────────────────────┘
          │  - practice_attempts │
          └──────────────────────┘
...
- Backend ↔ Judge0: **HTTP POST/GET** via RapidAPI  
...
Integrates Judge0 API to allow users to write and run code in the browser.

**Flow:**
1. User writes code in frontend Monaco-style editor
2. POST `/execute` with `source_code`, `language_id`, `stdin`
3. Backend base64-encodes source, submits to Judge0 → gets `token`
4. Backend polls Judge0 with token until status is not "Processing"
5. Returns `stdout`, `stderr`, `compile_output`, `time`, `memory`

**Supported Languages (Judge0 language_id):**
| Language | ID |
|----------|-----|
| Python 3 | 71 |
| C++ (GCC 9.2) | 54 |
...
**File:** `backend/services/judge0_service.py`, `backend/routers/compiler.py`
...
- Users submit code → executed via Judge0 → compared against expected output
...
# Step 2: Poll (repeat until status.id NOT IN [1, 2])
GET /submissions/{token}?base64_encoded=true&fields=stdout,stderr,status,time,memory,compile_output
Response: {
    "stdout": "<base64>",
    "stderr": "<base64>",
...
# 4 = Wrong Answer, 5 = TLE, 6 = Compile Error, 11 = Runtime Error
```

**Polling strategy:** Poll every **1 second**, max **10 attempts**. If still processing after 10s → return timeout error.
...
│   │   ├── judge0_service.py       # Judge0 compile/run
...
main          ← Production-ready, final stable code only
  └── dev     ← Integration branch; all features merge here first
        ...
        └── feature-ruman   ← Ruman's work (DB, deployment, judge0 proxy)
```
...
fix(judge0): handle polling timeout gracefully
...
| **Compiler** | ✅ Judge0 service | — | Proxy setup |
...
- [ ] Judge0 returning actual execution results
...
4. judge0_service.py:
   ...
   b. POST to Judge0 /submissions → get token
...
503 Service Unavailable → External API (Groq/Judge0) unreachable
```
...
| Timeout | Judge0 unreachable | "Compiler service temporarily unavailable" |
...
- **Judge0 code isolation** — all code runs in Judge0's sandboxed containers; no code runs on our server
